...
export const MainPage: React.FC = () => {
  ...
  const resetSets = () => {
    ...
    if (selectedSubCategory !== "") {
      ...
      instance
        .get(
          `/sets?page=1&size=10&sort_by_date=true&ascending=true&category_id=${categoryID}`
        )
        .then((response) => {
          setTotalSetPages(response.data.total_pages);
          const newCards = [];
      ...
      instance
        .get(
          `/folders?page=${pageFolder}&size=10&sort_by_date=true&ascending=true&category_id=${categoryID}`
        )
        .then((response) => {
          setTotalFolderPages(response.data.total_pages);
      ...
      instance
        .get(
          `/folders?page=${pageFolder}&size=10&sort_by_date=true&ascending=true`
        )
        .then((response) => {
          setTotalFolderPages(response.data.total_pages);
          const newFolderCards = [];
  ...
  useEffect(() => {
    setPageSet(1);
    instance
      .get(`/sets?page=${pageSet}&size=10&sort_by_date=true&ascending=true`)
      .then((response) => {
        setTotalSetPages(response.data.total_pages);
        const newCards = [...setCards];
        response.data.sets.forEach((card) => newCards.push(card));
  ...
  const changeCategory = (id: string, name: string) => {
    ...
    instance
      .get(
        `/sets?page=1&size=10&sort_by_date=true&ascending=true&category_id=${id}`
      )
      .then((response) => {
        setTotalSetPages(response.data.total_pages);
        const newCards = [];
  ...
  const changeSubCategory = (id: string, name: string) => {
    ...
    instance
      .get(
        `/sets?page=1&size=10&sort_by_date=true&ascending=true&category_id=${categoryID}&subcategory_id=${id}`
      )
      .then((response) => {
        setTotalSetPages(response.data.total_pages);
        const newCards = [];
